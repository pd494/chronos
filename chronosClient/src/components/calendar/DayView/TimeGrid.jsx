      ...
      <div className="flex-1 relative" style={{ height: `${gridHeight}px`, minHeight: `${gridHeight}px` }}>
        ...
        >
          ...
          {hours.map((hour) => (
            <div
              key={hour}
              className="day-hour-cell absolute left-0 right-0 cursor-default z-[2] transition-colors duration-200
              ...
              onDragLeave={(e) => {
                ...
                if (document.body.classList.contains('task-dragging')) {
                  ...
                  const stayingInCells = relatedTarget && relatedTarget.closest('.day-hour-cell')
                  const hasPendingDrop = pendingTodoPreviewRef.current
                  if (!stayingInCells && !hasPendingDrop) clearTodoDragPreview()
                }
              }}
            />
          ))}
