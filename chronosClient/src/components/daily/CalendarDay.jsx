  ...
  const [{ isOver, canDrop, isDuplicate }, drop] = useDrop(
    () => ({
      ...
      collect: (monitor) => ({
        ...
        isDuplicate: monitor.getItem()
          ? !canDropTaskOnDate(monitor.getItem().id, day.date)
          : false
      })
    }),
    [day.date, events]
  ...
  return (
    <div
      ...
      className={`day ${!day.isCurrentMonth ? 'other-month' : ''} ${
        ...
        isOver && isDuplicate ? 'duplicate-task' : ''
      } ${canDrop ? 'can-drop' : ''}`}
    >
      {monthLabel && (
        <div
          className="month-indicator"
          // Adding a bottom margin separates the label from the day number
