...
const WeekEvent = ({ event, hourHeight, dayStartHour, position }) => {
  ...
  const titleColor = isDeclined ? 'rgba(71, 85, 105, 0.4)' : (colors.text.startsWith('#') ? hexToRgba(colors.text, 0.9) : colors.text)
  const timeColor = isDeclined ? 'rgba(71, 85, 105, 0.4)' : (colors.text.startsWith('#') ? hexToRgba(colors.text, 0.7) : colors.text)
