    ...
    <div className="z-20 bg-white border-t border-gray-100 px-4 py-3 flex flex-col gap-2">
      <div className="flex items-center gap-3">
        <div className="relative" ref={colorPickerTriggerRef}>
          <button type="button" onClick={() => setShowColorPicker(!showColorPicker)} className="px-2 py-2 hover:bg-gray-50 transition-colors" title="Change color">
            <div className="w-4 h-4 rounded-full" style={{ backgroundColor: getColorHex(color) }} />
          </button>
        </div>
        <button type="button" onClick={() => setShowNotificationPicker(prev => !prev)} className="px-2 py-2 text-gray-500 hover:bg-gray-50 transition-colors" title="Add notification" ref={notificationTriggerRef}>
        ...
        <button type="button" onClick={() => setShowAsBusy(prev => !prev)} className="flex items-center gap-1 px-3 py-1 text-sm font-normal text-gray-700 hover:bg-gray-50 transition-colors" title={showAsBusy ? 'Show as busy' : 'Show as free'} style={{ minWidth: 64 }}>
          <span className={`w-2 h-2 rounded-full ${showAsBusy ? 'bg-red-500' : 'bg-green-500'}`} />
          <span>{showAsBusy ? 'Busy' : 'Free'}</span>
        </button>
        <button type="button" onClick={() => setIsPrivateEvent(prev => !prev)} className={`p-2 transition-colors ${isPrivateEvent ? 'text-gray-900' : 'text-gray-500 hover:text-gray-600'}`} aria-pressed={isPrivateEvent} title={isPrivateEvent ? 'Private event' : 'Public event'}>
          ...
          <div className="relative">
            <button type="button" onClick={() => setShowNotifyMembers(prev => !prev)}
              className={`flex items-center gap-1.5 px-3 py-1.5 text-sm shrink-0 transition-colors ${
                currentRSVPStatus === 'accepted' ? 'text-green-700' : currentRSVPStatus === 'declined' ? 'text-red-600' : 'text-gray-600'
              }`}>
